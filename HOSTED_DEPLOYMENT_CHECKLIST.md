...
8. Open `/health/version` after deployment and confirm version `2.10.0`, build `2026-09-18-v2.10.0-scheduler-retention-isolated-r7`, `nightly_scheduler_mode=dedicated_worker_expected`, and the latest run date/status. PostgreSQL deployments default the guarded web fallback on unless `PIPEFLOW_NIGHTLY_SCHEDULER=0` is explicitly set.
